...
#
# 短縮フラグは結合形（-dw / -de など）も対象にする。ただし値が連結された形
# （-b"text" / -L5 / -q.foo など）を誤って弾かないよう、判定するのは
# 「ダッシュに続く部分が英字だけ」の引数に限る。値が連結されていれば英字以外
# （空白・数字・記号）を含むため対象外になる。
# なお安全性の最終的な担保は exec 直前の環境変数の無害化（下部）であり、
# ここでの拒否は分かりやすいエラーを出すためのもの。
for a in "${args[@]}"; do
  case "$a" in
    --web|--web=*)
