...
# --- --web / -w を拒否 -------------------------------------------------------
# --web はブラウザを開いて操作させるため、注入した GH_TOKEN ではなく
# ブラウザにログイン中の個人アカウントで PR やコメントが作成される
# （＝ App 名義で操作するという前提が崩れる）。読み取り系でもトークンを
# 使わない以上このラッパーを通す意味が無いので、一律で拒否する。
for a in "${args[@]}"; do
  case "$a" in
    --web|--web=*)
      die "--web / -w は使えません（ブラウザ上の個人アカウントで操作されるため）: ${a}" 3 ;;
    -[!-]*)
      # 短縮フラグ（結合形 -dw / -wd も含む）に w が入っていれば拒否
      case "${a#-}" in
        *w*) die "--web / -w は使えません（ブラウザ上の個人アカウントで操作されるため）: ${a}" 3 ;;
      esac ;;
  esac
done
